...
Every job description that enters this system gets forced into the same YAML schema, normalized against a canonical tool list, and classified into one of five role archetypes. The analysis layer regenerates market-wide insights automatically. New JDs are fed from Clay, and the system gets smarter with every one it processes.
...
You could dump a pile of JDs into ChatGPT and get a summary. This is different — it's a structured data model with a processing pipeline. The skill isn't the prompting — it's the design of the harness: what fields matter, how to normalize messy data, where to enforce structure, and what gets regenerated vs. what persists.
...
| Folder | Purpose |
|---|---|
| `JDs/` | Parsed job descriptions — one file per role |
| `Dream/` | Aspirational stretch roles — gap analysis only |
| `Analysis/` | Generated market intelligence artifacts — regenerated by `/analyze` |
| `Hubs/Tools/` | Auto-generated hub pages for tools appearing in 3+ JDs |
| `Hubs/Archetypes/` | Hub pages for each role archetype |
| `Context/` | Living reference docs — Tags, Archetypes, Profile |
| `Raw/` | Original unmodified source data |
| `Examples/` | Gold standard parsing reference |
...
|---|---|
| **Tool Frequency Map** | Tools ranked by adoption — table-stakes (60%+), differentiator (25-59%), rare (<25%) |
| **Skills Tier Map** | Required vs. preferred skills with frequency tiers |
| **Role Archetype Clusters** | Archetypes with JD membership and distinguishing traits |
...
- **Tags.md** — Canonical tool names with alias mapping. Ensures "SFDC" and "Salesforce" resolve to the same node.
